...
Pipeline (two passes; the expensive pass runs once):
  ...
  * Pass 1 (analyze): parse + clean every trajectory, tokenize it once with the
    tools-aware chat template (untruncated) to get an exact ``n_tokens``,
...
turn/stop token and causes inference death-looping).

Usage:
    # Analysis-first: tokenize once, print the coverage curve, cache analyzed JSONL
    python examples/long_context_validation/qwen3_32b/data/prefilter_dataset.py \
    ...
    python examples/long_context_validation/qwen3_32b/data/prefilter_dataset.py \
        --model Qwen/Qwen3-32B --max_samples 20 --seq_length 16384
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model", type=str, required=True, help="Tokenizer path or HF id (e.g. Qwen/Qwen3-32B)")
    p.add_argument(
        "--chat_template",
        type=str,
        default=None,
